...
func Help() {
	...
	print("Initialize the linksym configuration file (.linksym.yaml+reset) to hold records of symlinks.", white, 2)
	print("", white, 0)
	print("add [target] [destination (Optional+reset)]", boldWhite, 1)
	print("Create a symlink for the specified path. Optionally takes a destination path for the symlink.", white, 2)
	print("", white, 0)
	print("record [target] [destination (Optional+reset)]", boldWhite, 1)
	print("Creates a record of symlink in .linksym.yaml, without actually creating symlink.", white, 2)
	print("", white, 0)
	print("remove [target(s+reset)...]", boldWhite, 1)
	print("Remove the symlink and restore the original file to its original path.", white, 2)
	print("", white, 0)
	print("restore [target(s+reset)...]", boldWhite, 1)
	print("Create symlink for specified target(s+reset) that has a record in .linksym.yaml configuration file.", white, 2)
	print("", white, 0)
